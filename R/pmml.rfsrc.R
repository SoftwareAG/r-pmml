...
#'
#'
#' @param model An rfsrc object returned by the function \code{rfsrc} with
...
#' @inheritParams pmml
#'
#' @return PMML representation of the rfsrc object.
#'
...
pmml.rfsrc <- function(model,
  ...
  miningModelNode <- append.XMLNode(miningModelNode, .pmmlMiningSchemaSurv(field, timeName = model$yvar.names[1], statusName = model$yvar.names[2], target = NULL, inactive = NULL, transformed = transforms, missing_value_replacement = missing_value_replacement))
  ...
  extensionNode <- append.XMLNode(
    extensionNode,
    xmlNode("Array", attrs = c(
  ...
  for (b in 1:numTrees)
  {
    ...
    treeModelNode <- xmlNode("TreeModel",
      attrs = c(
        modelName = treeName, functionName = "regression",
        algorithmName = "rsf",
    ...
    treeModelNode <- append.XMLNode(treeModelNode, .pmmlMiningSchemaSurv(field, timeName = model$yvar.names[1], statusName = model$yvar.names[2], NULL, missing_value_replacement = missing_value_replacement))
    ...
    if (rootParmID != 0) {
      ...
      recursiveOutput <- .rsfMakeTree(
        recursiveOutput, nativeArray,
...
.rsfMakeTree <- function(recursiveObject, nativeArray, predictorNames, b,
  ...
  if (b != nativeArray$treeID[recursiveObject$offset]) {
    stop(
      "Invalid nativeArray input record (treeID) at ",
  ...
  if (fwdSplitParameter == 0) {
    ...
    extensionNode <- append.XMLNode(
      extensionNode,
      xmlNode("Array", attrs = c(
        n = length(scorelist[, 2]),
        type = "double"
      ), paste(scorelist[, 2], collapse = " "))
    )
  ...
  if (daughter == -1) {
    ...
    stop(
      "Invalid parse direction encountered during recursion.",
