...
test_that("Error when attribute is NULL", {
  ...
  expect_error(add_data_field_attributes(
    ...
    field = "Sepal.Width"),"attribute must be a data.frame or vector.")
  
})

test_that("Error when field additions are not in model", {
  expect_error(add_data_field_attributes(fit_pmml, 
                            c(displayName = "FlowerWidth",
                              isCyclic = 1), "Foo.Bar"),
  ...
  expect_error(add_data_field_attributes(fit_pmml, attributes_2,
                                         namespace = "4_4"),
