...
test_that("AnomalyDetectioneModel/iForest PMML output matches R", {
  skip_on_cran()
  skip_on_ci()

  anomaly_threshold <- 0.6
  fit <- iForest(iris, nt = 7, phi = 30)
  p_fit <- pmml(fit, anomalyThreshold = anomaly_threshold)
  ...
  p_fit <- pmml(fit, transforms = box_obj, anomalyThreshold = anomaly_threshold)
