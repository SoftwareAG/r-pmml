...
test_that("Error message is displayed when using defunct function name", {
  ...
  expect_error(DiscretizeXform(iris_box,
  ...
  expect_error(FunctionXform(iris_box,
  ...
  expect_error(MapXform(audit_box, xform_info = "[Sex -> d_sex][string->integer]",
                        table = dummy_file, map_missing_to = "0"),
               "This function is defunct; use xform_map instead.")

  expect_error(MinMaxXform(iris_box),
               "This function is defunct; use xform_min_max instead.")

  expect_error(NormDiscreteXform (iris_box, xform_info = "Species"),
               "This function is defunct; use xform_norm_discrete instead.")

  expect_error(RenameVar(wrap_object = iris_box, xform_info = "Sepal.Width->SW"),
